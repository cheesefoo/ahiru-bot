...
export class DeepLCommand implements Command {
    ...
    public async execute(msg: Message, args: string[], data: EventData): Promise<void> {
        ...
        let text = args.slice(2).reduce((prev, cur, _index, _array) => { return prev + cur; })
        ...
        if (tl != undefined) { tl = `${emoji}:${tl}`; }
        await MessageUtils.send(msg.channel, tl);

    }

    private async ParseTranslations(translations: { detected_source_language: string; text: string; }): Promise<string> {
